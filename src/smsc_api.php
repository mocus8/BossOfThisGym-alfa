...
function get_sms_cost($phones, $message, $translit = 0, $format = 0, $sender = false, $query = "")
{
    ...
    $m = _smsc_send_cmd("send", "cost=1&phones=".urlencode($phones)."&mes=".urlencode($message).
                    ($format > 0 ? "&".$formats[$format] : "").($sender === false ? "" : ($format == 12 ? "bot=" : "&sender=").urlencode($sender)).
                    "&translit=".$translit.($query ? "&$query" : ""));
...
{
    $url = $_url = (SMSC_HTTPS ? "https" : "http")."://smsc.ru/sys/$cmd.php?".(SMSC_LOGIN ? "login=".urlencode(SMSC_LOGIN)."&psw=" : "apikey=").urlencode(SMSC_PASSWORD)."&fmt=1&charset=".SMSC_CHARSET."&".$arg;
